...
export async function generateDocumentoCobranza(data) {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({
        ...
        bufferPages: true,
        info: {
          Title: `Documento de Cobranza ${data.numero || ''}`,
          Author: COMPANY.name,
          Subject: 'Documento de Cobranza'
        }
      });
...
function renderDocumento(doc, data) {
...
function addCompanyHeader(doc, data) {
  ...
  doc.fontSize(13).font('Helvetica-Bold')
    .text('DOCUMENTO DE COBRANZA', rightColX, ryStart + 28, {
      width: rightColWidth, align: 'center'
    });

  if (data.numero) {
    doc.fontSize(12).font('Helvetica-Bold')
      .text(data.numero, rightColX, ryStart + 50, {
        width: rightColWidth, align: 'center'
      });
  }
...
function addServicesTable(doc, startY, data) {
  ...
  const costoEnvio = data.costoEnvio != null ? Number(data.costoEnvio) : 0;
  y = addTotalsTable(doc, y, data, subtotal, costoEnvio, symbol);

  return y;
}

function addTotalsTable(doc, startY, data, subtotal, costoEnvio, symbol) {
  ...
  doc.text('SUBTOTAL', xs[3] + 3, y + 4, { width: colWidths[3] - 6, align: 'center' });
  drawRect(doc, xs[4], y, colWidths[4], rowH);
  doc.text(`${symbol} ${formatNumber(subtotal)}`, xs[4] + 3, y + 4, { width: colWidths[4] - 6, align: 'right' });

  y += rowH;

  drawRect(doc, xs[1], y, sonWidth, rowH * 2);
...
function addObservationsBox(doc, startY, data) {
  ...
  if (y + boxH > PAGE.height - PAGE.marginBottom - 60) {
    doc.addPage();
    y = PAGE.marginTop;
  }
...
function addPageFooter(doc) {
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    if (isLastPage) {
      const disclaimer = 'NO VALIDO PARA CREDITO FISCAL, SOLO PARA FINES DE COBRANZA';
      doc.fontSize(9).font('Helvetica-Bold').fillColor(COLORS.red)
        .text(disclaimer, PAGE.marginLeft, PAGE.height - 45, {
          width: PAGE.contentWidth,
          align: 'center'
        });
    }

    const footerText = 'Representacion Impresa de DOCUMENTO DE COBRANZA';
